...
export const ThemeBottomBarSchema = z.object({
  ...
  /** 底欄底色（建議含 alpha） */
  background: z.string().default('rgba(252,248,238,0.78)'),
  borderColor: z.string().default('rgba(196,176,138,0.65)'),
  borderRadiusPx: z.number().min(8).max(28).default(14),
  boxShadow: z.string().default('0 10px 36px rgba(42,35,25,0.10)'),
  backdropBlurPx: z.number().min(0).max(40).default(14),
  labelInactiveColor: z.string().default('#7a6f5d'),
  /** 未填則使用 `accentColor` */
  labelActiveColor: z.string().optional(),
  /** Link/Space 開關在「開」時的軌道色；未填則使用 `accentColor` */
  switchCheckedBackground: z.string().optional(),
  iconInactiveColor: z.string().default('#7a6f5d'),
  iconActiveBackground: z.string().default('rgba(166,124,0,0.14)'),
  /** 未填則使用 `accentColor` */
  iconActiveColor: z.string().optional(),
  heartFavoriteBorder: z.string().default('#c4b08a'),
  heartFavoriteBg: z.string().default('rgba(139,41,66,0.10)'),
  heartFavoriteIcon: z.string().default('#8b2942'),
  heartDefaultBorder: z.string().default('#c4b08a'),
  heartDefaultBg: z.string().default('rgba(252,248,238,0.9)'),
  heartDefaultIcon: z.string().default('#5e5648'),
})
...
export const ThemeFolderTabsSchema = z.object({
  /** Tab 列底色（可含 alpha） */
  barBackground: z.string().default('rgba(252,248,238,0.88)'),
  barBorderColor: z.string().default('#c4b08a'),
  barBackdropBlurPx: z.number().min(0).max(24).default(8),
  inactiveColor: z.string().default('#7a6f5d'),
  inactiveHoverColor: z.string().default('#5e5648'),
  /** 未填則使用 `accentColor` */
...
export const ThemeSettingsSchema = z.object({
  ...
  accentColor: z.string().default('#8b2942'),
  backgroundColor: z.string().default('#f5efe4'),
  showBio: z.boolean().default(true),
  cardRadius: z.number().min(0).max(40).default(12),
