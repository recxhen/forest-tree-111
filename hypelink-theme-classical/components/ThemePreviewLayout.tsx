  ...
  const legacyLayout = (
    <div className="flex min-h-screen flex-col">
      ...
      {context.slots?.header ? (
        ...
        <header className="sticky top-0 z-30 flex h-14 shrink-0 items-center justify-between border-b border-hl-border bg-hl-parchment-elevated/92 px-4 backdrop-blur-sm sm:px-6">
          <a href="/" className="flex shrink-0 items-center">
            {context.profile.brandLogoUrl ? (
              ...
              <span className="text-lg font-bold tracking-wide text-hl-ink">
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1.5 rounded-full border border-hl-border bg-hl-parchment-shadow/70 py-1 pl-1 pr-2.5">
              <div className="flex h-7 w-7 items-center justify-center rounded-full border border-hl-gold/35 bg-hl-gold-soft text-[10px] font-bold" style={{ color: primary }}>
                {context.profile.name?.slice(0, 1) || '?'}
              </div>
              <span className="text-[12px] font-semibold text-hl-muted">
      ...
      <main className="relative flex min-h-0 flex-1 flex-col lg:flex-row">
        ...
        <div className="relative z-10 flex min-h-0 min-w-0 flex-1 flex-col lg:flex-row">
          ...
          <aside
            className={cx(
              'w-full shrink-0 border-hl-border bg-hl-parchment-elevated/96 backdrop-blur-[2px] lg:sticky lg:top-0 lg:max-h-screen lg:min-w-0 lg:overflow-y-auto lg:border-r lg:w-[min(400px,36vw)] lg:max-w-[400px] xl:w-[min(400px,32vw)]',
          ...
          >
            ...
            <div className="hidden w-full max-w-sm border-t border-hl-border px-9 pb-8 pt-6 lg:block">
              <p className="text-center text-[11px] font-semibold tracking-wide text-hl-muted">
                Powered by{' '}
                <a
                  href="https://hypelink.app"
                  className="underline decoration-hl-border underline-offset-2 hover:text-hl-ink"
                  style={{ color: primary }}
                >
                  HypeLink
                </a>
          ...
          >
            {currentTab === 'links' ? (
              <>
                ...
                <div className="flex min-h-[40vh] flex-1 items-center justify-center px-4 text-sm text-hl-muted">
                  尚未新增連結
                </div>
              )}
              </>
            ...
            {currentTab === 'space' ? (
              context.slots?.spaceEmbed ? (
                ...
                <div className="flex min-h-[50vh] flex-1 items-center justify-center px-6">
                  <p className="text-center text-sm text-hl-muted">
                    空間預覽（靜態模板不含 3D 嵌入）
                  </p>
                </div>
              )
            ...
            {currentTab === 'shop' ? (
              context.slots?.shopTab ? (
                ...
                <div className="flex min-h-[50vh] flex-1 items-center justify-center px-6 text-sm text-hl-muted">
                  Shop 即將推出
                </div>
              )
            ) : null}
